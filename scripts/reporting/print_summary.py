#!/usr/bin/env python3
"""Print a local human-readable experiment summary for selected framework runs.
...
from lab.reporting.framework import discover_framework_runs, is_none_like, normalize_name
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Print a local derived summary from framework run metrics. "
            "--auto resolves runs only from one explicit runs-root and is a diagnostic convenience."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Auto-discover the latest matching local baseline/attack runs from one runs-root. "
