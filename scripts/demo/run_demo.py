...
def stage_check_reports(reports_root: Path) -> int:
    """Assert core report files exist and are non-empty. Returns exit code."""
    required = [
...
def _build_stage3_sweep_command(
    *,
    args: argparse.Namespace,
    config_path: Path,
    runs_root: Path,
    ...
    command = [
        args.python_bin,
        str(REPO_ROOT / "scripts" / "run_unified.py"),
        "sweep",
...
def _build_stage3b_report_regen_command(
    *,
    args: argparse.Namespace,
    config_path: Path,
    runs_root: Path,
...
def main() -> int:  # noqa: C901 (acceptable complexity for a pipeline orchestrator)
    ...
    sweep_cmd = _build_stage3_sweep_command(
        args=args,
    ...
    # Stage 3b — Canonical Phase 4 regeneration through the public sweep surface,
    # requesting only the local core report bundle for the demo's isolated output root.
    reports_log = demo_root / "reports.log"
    report_regen_cmd = _build_stage3b_report_regen_command(
        args=args,
        config_path=config_path,
        runs_root=runs_root,
        reports_root=reports_root,
    )
    print(f"[REPORTS] Regenerating core Phase 4 bundle → log: {reports_log}")
    report_gen_rc = _run_subprocess(report_regen_cmd, reports_log, env=env)
    if report_gen_rc != 0:
        print(f"[REPORTS] core Phase 4 regeneration FAILED (exit {report_gen_rc}); see {reports_log}", file=sys.stderr)
