...
class PreprocessRandomResizeDefenseAdapter(BaseDefense):
    ...
    because YOLO anchors are optimized for full-resolution inputs. Phase 2 cycle ranking may
    look favorable (avg_conf proxy does not capture the drop), but Phase 4 mAP50 validation
    usually reveals the true cost. Always verify Phase 4 results before using in production.

    NOTE: Removed from ALL_DEFENSES in auto_cycle.py. The inherent mAP50 cost (~-0.25)
    from padding-induced anchor misalignment exceeds any attack-recovery benefit in practice.
    Available for direct use via --defenses random_resize if explicitly needed.
    """

    scale_factor_low: float = 0.9
    scale_factor_high: float = 1.0
    seed: int = -1  # -1 = non-deterministic per image
    ...
    def preprocess(self, image: np.ndarray, **kwargs: Any) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        padded = cv2.copyMakeBorder(
            resized, 0, h - new_h, 0, w - new_w, cv2.BORDER_CONSTANT, value=(0, 0, 0)
        )
    ...
    def postprocess(
        ...
        **kwargs: Any,
        ...
        return predictions, adapter_stage_metadata(
            "preprocess_random_resize", "postprocess", note="identity_postprocess"
        )
